...
class MeshDrawer {
	...
	draw(trans) {
		...
		gl.uniform3fv(this.lightPosLoc, [lightX, lightY, -4.0]);
...
function SetSpecularLight(slider) {
    meshDrawer.setSpecularIntensity(slider.value / 10.0);
}
...
void main() {
	...
	vec3 diffuseLight = max(dot(norm, lightDir), 0.0) * vec3(1.0);

	vec3 reflectDir = reflect(-lightDir, norm);
	float spec = 0.0;

	if (dot(viewDir, norm) > 0.0) {
		spec = pow(max(dot(viewDir, reflectDir), 0.0), 20.0);
	}

	vec3 specularLight = specularIntensity * spec * vec3(1.0);
	vec3 lighting = enableLighting ? (ambientLight + diffuseLight + specularLight) : vec3(1.0);
